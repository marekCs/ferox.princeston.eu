...
class Ga4GoogleAnalyticsSettings extends ConfigFormBase
{
    ...
    public function buildForm(array $form, FormStateInterface $form_state)
    {
        ...
        $form["ga4_google_analytics_api_settings"]["measurement_id"] = [
            ...
            "#default_value" => $config->get("measurement_id")
                ? $config->get("measurement_id")
                : "",
            "#description" =>
                'Enter your Google Analytics GA4 Measurement ID. The unique ID that looks like "G-XXXXXXXXXX" and is used to identify your property',
            "#required" => true,
        ];
        ...
        if ($config->get("ga4_access_pages")) {
            $this->condition->setConfiguration(
                $config->get("ga4_access_pages")
            );
        }

        $form["pages"] += $this->condition->buildConfigurationForm(
            $form["pages"],
            $form_state
        );
        $form["pages"]["negate"]["#type"] = "radios";
        $form["pages"]["negate"]["#default_value"] =
            (int) $form["pages"]["negate"]["#default_value"];
        ...
        $form["pages"]["negate"]["#options"] = [
            ...
            $this->t(
                'Don\'t use analytics with your site for the listed pages'
            ),
        ];
        ...
        $form["roles"]["ga4_access_roles"] = [
            ...
            "#default_value" => $config->get("ga4_access_roles")
                ? $config->get("ga4_access_roles")
                : [],
            "#options" => array_map(
    ...
    public function submitForm(array &$form, FormStateInterface $form_state)
    {
        ...
        $config->set(
            "ga4_access_roles",
            $form_state->getValue("ga4_access_roles")
        );
